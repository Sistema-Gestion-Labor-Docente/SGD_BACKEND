...
public class CalendarioController {
    ...
    public ResponseEntity<ApiResponse<Page<Calendario>>> findAll(
            @RequestParam(required = false) String nombreCalendario,
            @RequestParam(required = false) String estado,
            Pageable pageable) {
        ApiResponse<Page<Calendario>> response = calendarioService.obtenerTodos(nombreCalendario, estado, pageable);
        return ResponseEntity.status(response.getCodigo()).body(response);
    }

    @GetMapping("/{oid}")
    @Operation(summary = "Buscar calendario por ID", description = "Consulta un calendario específico por su ID")
    public ResponseEntity<ApiResponse<Calendario>> findByOid(@PathVariable Integer oid) {
        ApiResponse<Calendario> response = calendarioService.buscarPorId(oid);
        return ResponseEntity.status(response.getCodigo()).body(response);
    }

    @PostMapping
    @Operation(summary = "Guardar calendario", description = "Guarda un nuevo calendario")
    public ResponseEntity<ApiResponse<Calendario>> save(@RequestBody Calendario calendario) {
        ApiResponse<Calendario> response = calendarioService.guardar(calendario);
        return ResponseEntity.status(response.getCodigo()).body(response);
    }

    @PutMapping("/{id}")
    @Operation(summary = "Actualizar calendario", description = "Actualiza un calendario existente")
    public ResponseEntity<ApiResponse<Calendario>> update(@PathVariable Integer id, @RequestBody Calendario calendarioActualizado) {
        ApiResponse<Calendario> response = calendarioService.actualizar(id, calendarioActualizado);
        return ResponseEntity.status(response.getCodigo()).body(response);
    }
